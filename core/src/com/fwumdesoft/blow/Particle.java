...
public class Particle extends DrawingActor implements Poolable {
	...
	public Particle() {
		...
		set(xSpeed, ySpeed, lifetime);
		setOrigin(texture.getRegionWidth() / 2, texture.getRegionHeight() / 2);
	}
	
	public void set(float xSpeed, float ySpeed, int lifetime) {
		this.xSpeed = xSpeed;
		this.ySpeed = ySpeed;
	...
	public static void spawnCluster(Stage stage, int particles, float x, float y, float positionVariance, float xSpeed, float ySpeed, float speedVariance, int lifetime, int lifetimeVariance) {
		for(int i = 0; i < particles; i++) {
			Particle particle = pool.obtain();
			particle.set(xSpeed + ((float)Math.random() - 0.5f) * speedVariance, ySpeed + ((float)Math.random() - 0.5f) * speedVariance, lifetime);
